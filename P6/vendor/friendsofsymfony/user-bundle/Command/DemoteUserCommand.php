...
EOT
    ...
    protected function executeRoleCommand(UserManipulator $manipulator, OutputInterface $output, $username, $super, $role)
    {
        if ($super) {
            ...
            $output->writeln(sprintf('UserCred "%s" has been demoted as a simple user. This change will not apply until the user logs out and back in again.', $username));
        } else {
            if ($manipulator->removeRole($username, $role)) {
                ...
                $output->writeln(sprintf('UserCred "%s" didn\'t have "%s" role.', $username, $role));
            }
        }
    }
}
