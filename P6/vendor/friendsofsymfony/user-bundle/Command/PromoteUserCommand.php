...
EOT
    ...
    protected function executeRoleCommand(UserManipulator $manipulator, OutputInterface $output, $username, $super, $role)
    {
        if ($super) {
            ...
            $output->writeln(sprintf('UserCred "%s" has been promoted as a super administrator. This change will not apply until the user logs out and back in again.', $username));
        } else {
            if ($manipulator->addRole($username, $role)) {
                ...
                $output->writeln(sprintf('UserCred "%s" did already have "%s" role.', $username, $role));
            }
        }
    }
}
